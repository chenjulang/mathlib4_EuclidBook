...
/-- Order topology on a linear order `α` generated by the subbase of open intervals
`(a, ∞) = { x ∣ a < x }, (-∞ , b) = {x ∣ x < b}` for all `a, b` in `α`.

While the definition makes sense on any preorder,
it gives the desired result only for linear orders.
E.g., for product types the topology provided by this definition
is not equal to the product topology of order topologies.

For this reason, we require `α` to be a linear order instead of a preorder.

We do not register it as an instance
because many ordered types already have other `TopologicalSpace` instances.
While in most cases these instances are equal to `TopologicalSpace.order _`,
they are almost never definitionally equal.
Moreover, for some sets the order topology is not equal to the induced topology.
-/
protected def TopologicalSpace.order (α : Type*) [LinearOrder α] : TopologicalSpace α :=
  generateFrom (range Ioi ∪ range Iio)

@[reducible, deprecated] -- 2024-02-14
alias Preorder.topology := TopologicalSpace.order

/-- The order topology on a linear ordered type is the topology generated by open intervals.
We define it as a mixin. If you want to introduce the order topology on a preorder, use
`TopologicalSpace.order`. -/
class OrderTopology (α : Type*) [t : TopologicalSpace α] [LinearOrder α] : Prop where
  /-- The topology is generated by open intervals `Set.Ioi _` and `Set.Iio _`. -/
  topology_eq_order : t = .order α
...
instance : OrderTopology αᵒᵈ where
  topology_eq_order :=
    (@OrderTopology.topology_eq_order α _).trans <| congr_arg generateFrom <| union_comm _ _

theorem isOpen_iff_generate_intervals {s : Set α} :
    IsOpen s ↔ GenerateOpen (range Ioi ∪ range Iio) s := by
  rw [OrderTopology.topology_eq_order (α := α)]; rfl
...
theorem order_separated {a₁ a₂ : α} (h : a₁ < a₂) :
  ...
  ⟨Iio x, Ioi y, isOpen_iff_generate_intervals.2 <| .basic _ <| .inr <| mem_range_self _,
    isOpen_iff_generate_intervals.2 <| .basic _ <| .inl <| mem_range_self _, hx, hy, h⟩
...
@[deprecated isOpen_Ioi]
theorem isOpen_lt' (a : α) : IsOpen { b : α | a < b } := isOpen_Ioi

@[deprecated isOpen_Iio]
