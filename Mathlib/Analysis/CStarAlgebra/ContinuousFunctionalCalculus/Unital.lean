...
--lemma CFC.exists_pos_algebraMap_le_iff (a : A) [CompactSpace (spectrum R a)]
    --(h_non : (spectrum R a).Nonempty) (ha : p a := by cfc_tac) :
    --(∃ r > 0, algebraMap R A r ≤ a) ↔ (∀ x ∈ spectrum R a, 0 < x) := by
  --have h_cpct : IsCompact (spectrum R a) := isCompact_iff_compactSpace.mpr ‹_›
  --constructor
  --· rintro ⟨r, hr, hr'⟩
    --rw [algebraMap_le_iff_le_spectrum] at hr'
    --exact (hr.trans_le <| hr' · ·)
  --· intro h
    --obtain ⟨r, hr, hr'⟩ := h_cpct.exists_isMinOn h_non continuousOn_id
    --sorry
